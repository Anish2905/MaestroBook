...
export async function initDatabase() {
  const stmts = [
    ...
    `CREATE TABLE IF NOT EXISTS balance_overrides (override_id INTEGER PRIMARY KEY AUTOINCREMENT, party_id INTEGER NOT NULL REFERENCES parties(party_id), override_amount INTEGER NOT NULL, reason TEXT NOT NULL, is_deleted INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now')))`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_invoices_party_deleted ON invoices(party_id, is_deleted)`,
    `CREATE INDEX IF NOT EXISTS idx_transactions_party_deleted ON transactions(party_id, is_deleted)`,
    `CREATE INDEX IF NOT EXISTS idx_invoices_type ON invoices(invoice_type)`,
    `CREATE INDEX IF NOT EXISTS idx_transactions_type ON transactions(txn_type)`,
    `CREATE INDEX IF NOT EXISTS idx_balance_overrides_party_deleted ON balance_overrides(party_id, is_deleted)`,
